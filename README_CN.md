...
- Python 3.10+
- 已配好的 [OpenViking](https://github.com/volcengine/OpenViking)（有 `ov.conf`）
- OpenAI 兼容的 LLM API（审核/路由用）
- 搜索 API（推荐 Grok，也可用其他 OAI 兼容端点）
...
| 变量 | 说明 |
|------|------|
| `OPENVIKING_CONFIG_FILE` | 你已有的 `ov.conf` 路径 |
| `CURATOR_OAI_BASE` | OpenAI 兼容 API 地址（LLM 审核/路由）|
| `CURATOR_OAI_KEY` | API key |
| `CURATOR_GROK_KEY` | Grok key（用 `oai` provider 时不需要）|
