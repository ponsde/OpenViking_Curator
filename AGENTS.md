...
- cv_warnings 截断已修（4840aa0）：warnings 注入 sys_prompt
...
| cv_warnings 注入 sys_prompt | 彻底解决 external_text[:3000] 预算问题；末尾追加在正文长时会被截断（假安全感）|
...
- **cv_warnings**：warnings 注入 `judge_and_ingest()` 的 `sys_prompt`（`cv_warnings` 参数），完全不占 `external_text[:3000]` 预算。追加末尾的旧设计在正文接近 3000 chars 时会被截断，judge 看不到 warning 会 pass 本应拒绝的内容（假安全感）。
